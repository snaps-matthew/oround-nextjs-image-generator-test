...
class ImageComposer {
  ...
  async init(data:{
    ...
    if ((data.target === TargetType.STORE_LIST_1 || data.target === TargetType.STORE_DETAIL_2) && ['tinCase', 'smartTok', 'button', 'apparel'].includes(data.productEditInfo.groupDelimiterName)) await imageTextSaver(data.thumbnailImage.toDataURL(), 'patternImage');
