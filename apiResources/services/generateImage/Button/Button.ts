...
class Button extends ImageComposer {
  ...
  async composite() {
    ...
    if (this.target === TargetType.STORE_DETAIL_2 || this.target === TargetType.STORE_LIST_1) {
      ...
      return await createImageOfModelView({ categoryName, productCode, productColor, productSize, directionCode, artworkWidth, artworkHeight, thumbnailImage, canvas, printPosition, optionInfo, target })
